...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Cosimo Corrado';
    var email     = grunt.option('email')  || 'sparkie79@gmail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new eastron name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/eastron.png')) {
            fs.renameSync(__dirname + '/admin/eastron.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/eastron.html')) {
            fs.renameSync(__dirname + '/widgets/eastron.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/eastron/js/eastron.js')) {
            fs.renameSync(__dirname + '/widgets/eastron/js/eastron.js', __dirname + '/widgets/eastron/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/eastron')) {
            fs.renameSync(__dirname + '/widgets/eastron',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Eastron/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Eastron'
                        },
                        {
                            match: /Cosimo Corrado/g,
                            replacement: author
                        },
                        {
                            match: /sparkie79@gmail.com/g,
                            replacement: email
                        }
                    ]
                },
